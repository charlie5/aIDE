...
package body aIDE.Editor.of_access_type
is
   use Gtk.Builder,
   ...
   function on_type_name_Entry_leave (the_Entry : access Gtk_Entry_Record'Class;
                                      Target    : in     AdaM.a_Type.access_type.view) return Boolean
   is
...
--        Target.index_Indication.Last_is (the_Text);
   ...
   procedure on_index_type_Button_clicked (the_Entry  : access Gtk_Button_Record'Class;
                                                the_Editor : in     aIDE.Editor.of_access_type.view) --  return Boolean
   is
--        the_Text : constant String := the_Entry.get_Text;
   begin
      null;
...
--                                     Target     => the_Editor.Target.component_Indication.main_Type);
   ...
   procedure on_rid_Button_clicked (the_Button : access Gtk_Button_Record'Class;
                                    the_Editor : in     aIDE.Editor.of_access_type.view)
   is
...
--        Self.Target.is_Constrained (Now => False);
   ...
   package body Forge
   is
      function to_Editor (the_Target : in AdaM.a_Type.access_type.view) return View
      is
         use AdaM,
             Glib;

         Self        : constant Editor.of_access_type.view := new Editor.of_access_type.item;
      ...
      begin
         ...
         Result := the_Builder.Add_From_File ("glade/editor/access_type_editor.glade", Error'Access);
         ...
         Self.modifier_Label := Gtk_Label (the_Builder.get_Object ("modifier_Label"));
         Self.not_null_Label := Gtk_Label (the_Builder.get_Object ("not_null_Label"));

         Self.subeditor_Box  := gtk_Box    (the_Builder.get_Object ("subeditor_Box"));
...
--           Self.element_type_Button := Gtk_Button (the_Builder.get_Object ("element_type_Button"));
         ...
         Self.subeditor_Box.pack_Start (Editor.of_subtype_indication.Forge.to_Editor (Self.Target.Indication,
                                                                                      is_in_unconstrained_Array => False).top_Widget);
...
--        literal_Editor : aIDE.Editor.of_enumeration_literal.view;
   begin
      if Self.Target.has_not_Null then   Self.not_null_Label.show;
                                  else   Self.not_null_Label.hide;
      end if;

      case Self.Target.Modifier
      is
         when None              =>   Self.modifier_Label.hide;
         when all_Modifier      =>   Self.modifier_Label.show;
                                     Self.modifier_Label.set_Text (" all ");
         when constant_Modifier =>   Self.modifier_Label.show;
                                     Self.modifier_Label.set_Text (" constant ");
      end case;
...
end aIDE.Editor.of_access_type;
